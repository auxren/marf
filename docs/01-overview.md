...
module. See [Section shift](08-section-shift.md) for how to point the two
...
- a **Voltage** level (from its slider, or from an external input), with options
  for full or limited (octave) range and semitone quantizing;
...
expander the module has 16 stages, with a [section‑shift](08-section-shift.md)
...
rate is 32 kHz, and step times match the panel legend. Later maintenance work
added reliability fixes (accurate timing delays, a watchdog), checksummed
memory, and a scale‑aware quantizer engine. See `RELEASE_NOTES.txt`.
