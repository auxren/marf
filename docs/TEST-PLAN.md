...
- [ ] **B3** — *(Optional)* **LED-only swap** (OUTPUT PULSE switches): **Pulse 2 up** swaps just the Pulse 1/2 LEDs, **Pulse 1 up** = normal. Persists across reboot (enable a stage's Pulse 1 → the swapped LED lights).
- [ ] **B4** — *(Optional)* **Channel swap** (Time Source switch): **External up** swaps the switch inputs + output jacks (and their LEDs), **Internal up** = normal. Independent of B3; persists across reboot.
